...
conn = pymysql.connect(host='localhost',user='test123', passwd='test123', db='aiotdb')
...
threshold =100
c.execute('update value set status=0 where value>'+str(threshold))
conn.commit()
#results = c.fetchall()
#print(type(results))
#print(results[:10])
...
with gzip.GzipFile('./svc.pgz','w')as f:
    pickle.dump(sv, f)

with gzip.GzipFile('./lr.pgz','w')as f:
    pickle.dump(model, f)
